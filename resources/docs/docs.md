...
The `home-routes` handler in the `my-pharmacy.routes.home` namespace
...
The `home-page` function will in turn call the `my-pharmacy.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `my-pharmacy.handler` namespace:
...
Request middleware functions are located under the `my-pharmacy.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `my-pharmacy.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `my-pharmacy.db.core` in some other namespace.
